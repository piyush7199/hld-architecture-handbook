...
Imagine you're running a library with a single copy of a popular book. If that copy is checked out, damaged, or lost, no
one can read it.
...
**Data Replication** in distributed systems works the same way: creating multiple copies of data across different
machines (nodes) to ensure:
...
**Replication** is the process of maintaining multiple copies of the same data across different nodes (servers) in a
distributed system.
...
```
Write:
Client → Master → Write to Master
              → Replicate to Slaves (async or sync)

Read:
Client → Slave (read-only, no writes)
```
...
```
Write:
Client → Master A → Write to Master A
                  → Replicate to Master B, C, D

Client → Master B → Write to Master B
                  → Replicate to Master A, C, D
```
...
Client → Leader → Propose write
                → Consensus (majority of followers agree)
...
```
Client → Master → Write to Master
              → Replicate to Replica 1 (wait for ack)
              → Replicate to Replica 2 (wait for ack)
              → All acks received → Confirm to client
```
...
Client → Master → Write to Master
              ...
              → Replicate to Replicas (async, background)
```
...
Client → Master → Write to Master
              → Replicate to Replica 1 (wait for ack)
              → Replicate to Replica 2 (async, don't wait)
              → At least 1 ack received → Confirm to client
```
...
- Network latency: US ↔ Europe = 100ms, US ↔ Asia = 200ms
...
- **Total:** 3 regions × 3 servers = 9 servers
...
#### ✅ Step 2: Replication Synchronization
...
```
Write (US Master):
Client → US Master → Write confirmed (10ms)
                  → Replicate to replicas (async, background)
                  → Europe replicas updated (~100ms later)
                  → Asia replicas updated (~200ms later)
```
...
#### ✅ Step 3: Read-Your-Writes Consistency

**Problem:** User updates profile in US, reads from Asia replica → sees stale data
...
```
User in US:
  → Writes go to US Master
  → Reads go to US Replicas (same region, <1s lag)
  → Read-your-writes guaranteed (same region)
```
...
Else:
  → Read from Replica (acceptable staleness)
```
...
With 3 Replicas (Master + 2 Replicas):
  → Master failure: 1/3 chance, 30s failover
  → Replica failure: 2/3 chance, 0s impact (other replicas handle)
  → Total downtime: ~10 minutes/year
  → Availability: 99.998% (exceeds 99.99% requirement)
```
...
6 Read Replicas: 6 × $250 = $1,500/month
Total: $2,000/month
```
...
```
User (US) → US Master → Write confirmed (10ms)
...
```
User (US) → US Replica → Read (5ms, <1s lag, read-your-writes)
```

**3. Read Profile (Europe User):**

```
User (Europe) → Europe Replica → Read (5ms, 1-2s lag)
```

**4. Master Failure:**

```
US Master fails → Promote US Replica 1 to Master (30s)
               → Update routing
               → System continues (99.99% availability)
```
...
| **3 Replicas per Region**    | 99.99% availability           | Storage cost (3× data)                |
| **Read Replicas**            | 10× read capacity             | Eventual consistency                  |
...
- **Read Capacity:** 300,000/sec (6 replicas × 50k/sec, exceeds 100k requirement)

**Result:**

- ✅ Handles read/write load
- ✅ Meets availability (99.99%)
- ✅ Read-your-writes consistency (geographic routing)
