...
1. **Hard-Coded Address:** "John is always in Room 305" — but what if John moves to a different room? You'd need to update every reference.
2. **Service Directory:** Ask the receptionist "Where is John?" and they tell you the current room number. If John moves, only the directory needs updating.

**Service Discovery** in microservices works the same way. Instead of hard-coding service addresses (IPs, ports), services register themselves in a **service registry** (directory), and other services query the registry to find them dynamically.
...
**Service Discovery** is the automatic detection of services and their network locations (IP addresses and ports) in a distributed system. It consists of two main components:
...
```
Service A → Service B (hard-coded: 10.0.1.5:8080)
...
**Flow:**
```
1. Client → Service Registry: "Where is User Service?"
2. Service Registry → Client: [instance1, instance2, instance3]
3. Client → Load Balancer: Select instance (round-robin, etc.)
4. Client → Selected Instance: Make request
```
...
**Flow:**
```
1. Client → Load Balancer: Request to "user-service"
2. Load Balancer → Service Registry: "Where is user-service?"
3. Service Registry → Load Balancer: [instance1, instance2, instance3]
4. Load Balancer → Selected Instance: Route request
5. Selected Instance → Load Balancer → Client: Response
```
...
2. Service → Registry: Heartbeat every 30 seconds
3. Service stops → Deregister from registry
```
...
2. Service Registrar → Discovers service (via deployment system)
3. Service Registrar → Service Registry: Register service
4. Service Registrar → Monitors service health
5. Service stops → Service Registrar → Deregister
```
...
You are designing a microservices architecture with 50 services. Each service has multiple instances (3-10 instances per service) that are dynamically scaled based on load. Services are deployed in Kubernetes and can be moved between nodes, causing IP addresses to change.
...
#### ✅ Step 1: Choose Service Discovery Pattern
...
**Architecture:**
```
Services → Kubernetes Service (DNS) → Pods (instances)
```

#### ✅ Step 2: Kubernetes Service Configuration
...
**Flow:**
```
1. Pod starts → Kubernetes adds to service
2. Service → kube-proxy: Update load balancer
3. Requests to service → Routed to pod
4. Pod terminates → Kubernetes removes from service
```

#### ✅ Step 4: Service Discovery Mechanism
...
**Load Balancing Flow:**
```
Request → Service IP → kube-proxy → Select Pod (round-robin)
→ Pod 1, Pod 2, Pod 3 (rotated)
```
...
**Flow:**
```
1. Pod starts → Readiness probe runs
2. If ready → Pod added to service (traffic routed)
3. If not ready → Pod not in service (no traffic)
4. Liveness probe runs periodically
5. If fails → Pod restarted
6. If pod crashes → Removed from service automatically
```

#### ✅ Step 7: Handling Service Scaling
...
**Flow:**
```
1. CPU usage > 70% → HPA adds pod
2. New pod starts → Automatically added to service
3. kube-proxy → Updates load balancer
4. Traffic → Distributed to new pod
5. CPU usage < 70% → HPA removes pod
6. Pod terminates → Automatically removed from service
```
...
#### ✅ Step 8: Multi-Region Service Discovery
...
```
Region: US
  └─ Kubernetes Cluster (local service discovery)
...
Consul (Cross-Region):
  └─ Services register for cross-region discovery
```
...
2. Service A → DNS: Query "service-b"
3. DNS → Returns: Service B Cluster IP
4. Service A → Service B Cluster IP: HTTP/gRPC request
5. kube-proxy → Load balances to healthy pod
6. Pod → Processes request → Returns response
```

**Health Check Flow:**
```
1. Kubernetes → Pod: Readiness probe every 5 seconds
2. If healthy → Pod in service (traffic routed)
3. If unhealthy → Pod removed from service (no traffic)
4. Kubernetes → Pod: Liveness probe every 10 seconds
5. If dead → Pod restarted
```
