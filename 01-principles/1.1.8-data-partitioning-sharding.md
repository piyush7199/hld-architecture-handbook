...
Imagine you're running a library with millions of books. Instead of storing all books in one massive room (which becomes
impossible to navigate), you **partition** the books:
...
**Data Partitioning (Sharding)** is the same concept for databases: splitting a large dataset into smaller, manageable
pieces distributed across multiple machines. Each machine (shard) holds a subset of the data, allowing the system to
scale horizontally beyond the limits of a single server.
...
**Partitioning (Sharding)** is the process of dividing a large database or dataset into smaller, independent pieces
called **partitions** or **shards**. Each shard is stored on a separate machine and operates independently.
...
## ⚠️ Crucial Production Caveats & Sharding Anti-Patterns

### 1. The "Celebrity" / Hotspot Shard Nightmare
- **Problem:** Even if a hash function distributes $1,000,000$ users evenly across 10 shards, a single viral user (e.g., Elon Musk or Justin Bieber with 100M followers) will direct $99\%$ of traffic to the single shard storing their data (`user_id = 42`).
- **Mitigation:**
  1. **Compound Shard Keys:** Combine `user_id` with a synthetic salt (`user_id + '_' + (tweet_id % 10)`).
  2. **Dedicated Celebrity Cache Tier:** Route reads for high-cardinality entities to an in-memory Redis L1/L2 cache cluster, bypassing the sharded database entirely.

### 2. The Scatter-Gather Latency Explosion
- **Problem:** If a query does not include the `shard_key` (e.g., `SELECT * FROM orders WHERE status = 'PENDING'`), the application router must broadcast the query to **every single shard in the cluster** and aggregate results (Scatter-Gather).
- **Caveat:** Scatter-gather queries exhibit **tail latency amplification**—the total response time is bounded by the *slowest single shard in the cluster*.
- **Mitigation:** Maintain **Secondary Indexes via Global Index Table** or replicate data asynchronously to a dedicated search cluster (Elasticsearch / ClickHouse) for non-shard-key queries.

### 3. Live Resharding Operational Overhead
- **Problem:** When changing shard count from $N=4$ to $N=8$, standard `hash(key) % N` remaps **80% of all keys**, causing catastrophic cache invalidation and network saturation during live data migration.
- **Mitigation:** Always use **Consistent Hashing** or **Virtual Buckets** (e.g., Vitess / DynamoDB), where pre-allocated virtual partitions are moved between nodes without re-hashing all keys.
...
- Example: User IDs 1-1M → Shard 1, 1M-2M → Shard 2, etc.
...
```
❌ Bad: Partition by "status" (active/inactive)
   → 90% of data in "active" shard (hotspotting)

✅ Good: Partition by "user_id"
   → Even distribution, matches query pattern (get user by ID)
```
...
Range Partitioning:
→ Query only relevant shards (recent date ranges)
→ Merge results

Hash Partitioning:
→ Query ALL shards (data scattered)
→ Merge results (expensive)
```
...
You are designing a social media platform's user post storage system. The system must store 10 billion posts and handle
...
#### ✅ Step 1: Primary Partitioning Strategy
...
#### ✅ Step 2: Handling User Timeline Queries

**Problem:** "Get posts by user_id" requires querying all 100 shards (expensive)

**Solution: Secondary Index (User → Posts Mapping)**
...
- Each shard stores: user_id → [post_id1, post_id2, ...]
- Query: Get user's post_ids from index shard → Fetch posts from post shards
```
...
- Index: user_id → [post_id1, post_id2, ...]
...
#### ✅ Step 3: Handling Recent Posts Query
...
→ Returns top 100 recent post_ids
→ Fetch posts from relevant shards
```
...
└──────────────────────┬──────────────────────────────────┘
        ...
        ▼              ▼              ▼
┌──────────────┐ ┌──────────┐ ┌──────────────┐
│ Post Shards  │ │   Redis  │ │ User-Posts  │
│ (100 shards) │ │  Cache   │ │ Index Shards │
│              │ │          │ │ (100 shards) │
│ Hash by      │ │ Hot Posts│ │ Hash by      │
│ post_id      │ │          │ │ user_id      │
└──────────────┘ └──────────┘ └──────────────┘
```
...
```
post_id → hash(post_id) % 100 → Shard N → Return post
...
```
user_id → hash(user_id) % 100 → Index Shard → Get post_ids
→ Parallel fetch from post shards → Merge results
...
```
→ Redis Sorted Set → Get top 100 post_ids
→ Parallel fetch from post shards → Merge results
...
| **Separate User-Posts Index** | Efficient user timeline queries         | Two-step query (index → posts)             |
...
- **Throughput:** 1M reads/sec (100 shards × 10k reads/sec)
...
- ✅ Handles user timeline and recent posts
- ✅ Scales horizontally
- ✅ Handles hot keys with caching
