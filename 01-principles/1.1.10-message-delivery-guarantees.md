...
1. **At-Most-Once Delivery:** The post office might lose your letter, but if it arrives, it arrives only once. You might not get confirmation it was delivered.
2. **At-Least-Once Delivery:** The post office guarantees your letter will arrive, but if there's any uncertainty (like a network glitch), they'll send it again. You might receive duplicates.
3. **Exactly-Once Delivery:** The post office guarantees your letter arrives exactly once—no loss, no duplicates. This is the hardest to achieve and requires coordination.

**Message Delivery Guarantees** in distributed messaging systems work the same way. They define what happens to messages when systems fail, networks partition, or services restart.
...
**Message Delivery Guarantees** specify the behavior of a messaging system regarding message delivery in the presence of failures:
...
**Trade-offs:**
- ✅ **Simple:** No retry or deduplication logic
- ✅ **Fast:** No overhead from retries
- ❌ **Data Loss:** Messages can be lost
- ❌ **Unreliable:** Not suitable for critical operations
...
**Flow:**
```
1. Producer sends message → Queue stores message
2. Queue delivers to consumer
3. Consumer processes message
4. Consumer sends ACK → Queue removes message
5. If ACK not received (timeout/crash):
   → Queue retries message (duplicate delivery)
```
...
**Trade-offs:**
- ✅ **Reliable:** Messages never lost
- ✅ **Simple:** Easier than exactly-once
- ❌ **Duplicates:** Must handle with idempotency
- ❌ **Retry Overhead:** May cause duplicate processing

### 3. Exactly-Once Delivery

**Definition:** Each message is delivered exactly once—never lost, never duplicated.
...
**Trade-offs:**
- ✅ **Perfect:** No loss, no duplicates
- ✅ **Correct:** Simplest for consumers (no duplicate handling)
- ❌ **Complex:** Requires coordination, state management
- ❌ **Expensive:** Higher latency, resource usage
- ❌ **Not Always Possible:** Some systems can't guarantee exactly-once

### 4. Delivery Guarantee Comparison

| Guarantee | Message Loss | Duplicates | Complexity | Latency | Use Case |
|-----------|--------------|------------|------------|---------|----------|
| **At-Most-Once** | ✅ Possible | ❌ Never | ✅ Simple | ✅ Low | Non-critical data |
| **At-Least-Once** | ❌ Never | ✅ Possible | ⚠️ Medium | ⚠️ Medium | Critical data (with idempotency) |
| **Exactly-Once** | ❌ Never | ❌ Never | ❌ Complex | ❌ High | Financial, critical state |
...
**Implementation:**
```
Payment Request → Kafka (at-least-once)
                → Payment Processor (idempotent with key)
                → Result: Each payment processed exactly once
```
...
**Implementation:**
```
Order Request → RabbitMQ (at-least-once)
              → Order Service (check if order_id exists)
              → Result: Each order created exactly once
```
...
**Implementation:**
```
Metrics → UDP (fire-and-forget)
        → Metrics Collector
        → Result: Most metrics collected, some may be lost
```
...
**Implementation:**
```
Transaction Event → Kafka (idempotent producer)
                  → Event Store (transactional)
                  → Result: Each event stored exactly once
```
...
You are designing a notification service that sends emails to users. The service receives notification requests from multiple microservices over unreliable networks.
...
#### ✅ Step 1: Choose Delivery Guarantee
...
**Architecture:**
```
Microservices → Kafka (at-least-once) → Notification Workers (idempotent)
```
...
#### ✅ Step 4: Notification Worker Design
...
#### ✅ Step 5: Handling Email Service Failures

**Problem:** Email service fails → notifications retried → duplicate emails
...
2. Check idempotency: Already sent? → Skip
3. If not sent:
   a. Send email
...
Worker 1: Check idempotency → Not found → Send email → Store result
Worker 2: Check idempotency → Not found → Send email → Store result
...
#### ✅ Step 6: Handling Kafka Consumer Failures

**Problem:** Consumer crashes → message not acknowledged → Kafka retries → duplicate
...
```
Worker receives message → Processes → Crashes before ACK
→ Kafka redelivers message → Worker checks idempotency → Already sent → Skip
...
→ Kafka redelivers message → Worker retries → Sends email
→ Result: Email sent (no duplicate if idempotency works)
```
...
┌─────────────────────────────────────────────────────────┐
│                    Kafka Cluster                         │
│  Topic: notifications (100 partitions, RF=3)           │
...
│ (Partition 0)│ │ (Part 1)  │ │ (Partition 99)│
└──────┬───────┘ └─────┬────┘ └──────┬───────┘
       │               │              │
       └───────┬───────┴──────┬───────┘
               │              │
...
**Flow:**
```
1. Microservice → Kafka: Send notification (at-least-once)
2. Kafka → Worker: Deliver message
3. Worker → Redis: Check idempotency
4. If not processed:
   a. Worker → Email Service: Send email
   b. Worker → Redis: Store idempotency result
   c. Worker → Kafka: Acknowledge message
5. If already processed:
   a. Worker → Skip (return cached email_id)
   b. Worker → Kafka: Acknowledge message
```
...
**Performance:**
- **Throughput:** 100,000 notifications/sec (100 partitions × 1k/sec)
...
**Result:**
- ✅ Every notification sent (at-least-once)
- ✅ No duplicate emails (idempotency)
- ✅ Handles failures gracefully
- ✅ Meets latency requirement (<5s)
- ✅ Scales to 100k notifications/sec
