...
Shows the complete Saga pattern with three steps: reserve inventory, charge customer, create order. All steps succeed in
this happy path.
...
Shows the Saga compensation flow when payment fails. The system automatically rolls back the reservation and returns
inventory to Redis.
...
Shows the background worker process that scans for expired reservations (TTL elapsed) and automatically returns
inventory to Redis.
...
Shows how two users attempting to reserve the last item simultaneously are handled atomically by Redis Lua script,
...
Shows how split counter optimization distributes load across 10 Redis keys, with fallback to next shard if current shard
is empty.
