...
This document contains detailed sequence diagrams showing interaction flows for the Collaborative Editor system,
including real-time collaboration, conflict resolution, offline sync, and failure scenarios.
...
Shows the complete flow when a user opens an existing document, including authentication, permission check, document
retrieval from cache, and WebSocket subscription.
...
4. **Server Update While Offline** (T3): Another user inserts "Wonderful " at position 6 → Server state: "Hello
   Wonderful World" (version 43)
