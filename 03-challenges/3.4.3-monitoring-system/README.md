...
Design a distributed monitoring system that collects, stores, and analyzes metrics from millions of endpoints (servers, containers, applications) in real-time. The system must handle **100M writes/sec**, store **10 PB** of compressed data, and provide **real-time alerting** (<5 seconds) with **fast dashboard queries** (<1 second).
...
**Key Insight:** System is **write-heavy** (100M writes/sec vs 16k reads/sec). Must optimize for sequential writes and time-based compression.
...
**Partitioning:** By `(metric_name, labels)` to group all data points for a specific time series. Cluster by `timestamp DESC` for efficient time-range queries.
...
**Benefits:** Reduces storage from 100 PB to 10 PB. Speeds up long-range queries by 3600x (use 1-hour rollup instead of raw).
...
**Decision:** **M3DB** for write-heavy workloads with Prometheus compatibility. *(See [this-over-that.md](./this-over-that.md) for analysis.)*
...
- [1.1.2 Latency, Throughput, and Scale](../../01-principles/1.1.2-latency-throughput-scale.md) - Understanding QPS and throughput calculations
- [1.1.4 Data Consistency Models](../../01-principles/1.1.4-data-consistency-models.md) - Eventual consistency in multi-region
- [1.1.5 Back-of-Envelope Calculations](../../01-principles/1.1.5-back-of-envelope-calculations.md) - Storage and bandwidth estimation
...
- [2.3.5 Batch vs Stream Processing](../../02-components/2.3.5-batch-vs-stream-processing.md) - Stream processing for alerting
...
**Total:** This comprehensive design covers building a distributed monitoring system handling 100M writes/sec, 10 PB storage, real-time alerting, and fast dashboards at scale.
