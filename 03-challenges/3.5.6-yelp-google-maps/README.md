...
```
San Francisco: lat=37.7749, lng=-122.4194 → geohash = "9q8yy9m"

Precision levels:
- 3 chars: 9q8 → ~156 km × 156 km (city-level)
- 5 chars: 9q8yy → ~4.9 km × 4.9 km (neighborhood)
- 7 chars: 9q8yy9m → ~153 m × 153 m (POI-level)
- 9 chars: 9q8yy9mj → ~4.8 m × 4.8 m (building-level)
```
...
|---------------------------|----------------------------------|---------------------------|-----------------------|
| **Simplicity**            | ✅ String prefix matching         | ❌ Complex hexagonal math  | ❌ Complex SQL queries |
| **Elasticsearch Support** | ✅ Native geo_point               | ⚠️ Custom implementation  | ❌ Database-only       |
| **Boundary Issues**       | ⚠️ Edge cases at cell boundaries | ✅ Hexagons tile perfectly | ✅ No boundaries       |
| **Sharding**              | ✅ Easy (prefix-based)            | ⚠️ Custom sharding logic  | ❌ Database-dependent  |
| **Adoption**              | ✅ Widely supported               | ❌ Uber-specific           | ✅ Industry standard   |
...
- **Example**: `9q8` → Shard 1 (West Coast USA), `drm` → Shard 2 (London)
...
1. **Optimize Geohash Precision**: Use appropriate precision based on radius
    - Small radius (<1km) → 7-char Geohash (fewer cells to query)
    - Large radius (>10km) → 5-char Geohash (coarser, fewer cells)
...
- **Nodes**: 15 nodes per cluster (5 shards × 3 replicas)
...
- ❌ Slower queries (10-50ms vs <10ms for Elasticsearch)
- ❌ Write bottleneck (10K writes/sec max)
- ❌ Complex SQL queries (harder to maintain)
...
- ✅ Ultra-fast (sub-millisecond queries)
- ✅ Simple API (GEOADD, GEORADIUS)
- ✅ In-memory (no disk I/O)

**Cons:**

- ❌ Limited filtering (no category, rating filters)
- ❌ Memory constraints (50M POIs × 50 bytes = 2.5GB)
- ❌ No full-text search
...
- ✅ Better boundaries (hexagons tile perfectly)
- ✅ Uniform distance (all neighbors equidistant)
- ✅ No edge artifacts

**Cons:**

- ❌ Complex implementation (custom code)
- ❌ Less tooling (fewer libraries)
...
|----------------------------------------------------|----------------------------------------------------------|
| ✅ **Fast Queries** (<100ms)                        | ❌ **Complex Architecture** (multiple databases)          |
| ✅ **Scalability** (50M POIs)                       | ❌ **Eventual Consistency** (POI updates propagate async) |
| ✅ **Flexible Filtering** (category, rating, hours) | ❌ **Storage Overhead** (denormalized review data)        |
| ✅ **Geographic Distribution** (multi-region)       | ❌ **Cross-Region Latency** (200ms for global queries)    |
| ✅ **Cache Performance** (80% hit rate)             | ❌ **Stale Data** (5-minute TTL)                          |
| ✅ **Horizontal Scaling** (sharding by Geohash)     | ❌ **Hotspot Risk** (dense areas need sub-sharding)       |
...
| **Complex Filtering**  | ✅ Native (category + rating + hours) | ❌ Complex SQL                |
| **Full-Text Search**   | ✅ Inverted index                     | ⚠️ Requires additional index |
| **Horizontal Scaling** | ✅ Native sharding                    | ❌ Manual sharding            |
| **ACID**               | ❌ Eventual consistency               | ✅ ACID guarantees            |
...
3. **Elasticsearch**: 15-node cluster (5 shards × 3 replicas)
...
- Effective QPS to Elasticsearch: 11K × 20% = 2.2K QPS
...
| **MongoDB**            | 3-node replica set × 32 shards | Review storage                   |
