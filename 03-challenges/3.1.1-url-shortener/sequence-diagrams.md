...
**Race Condition Protection:** Database UNIQUE constraint is the source of truth. Cache check is just an optimization to
fail fast.
...
**Performance:** ~50ms total latency (DB query ~30ms, cache write ~5ms, redirect ~10ms). Still acceptable for 10% of
traffic.
...
**Key Design:** Fully asynchronous to not impact redirect latency. Event loss is acceptable (monitoring/analytics, not
critical path).
...
**Capacity:** 4,096 IDs per millisecond per worker = ~4.1 million IDs/second per worker. Multiple workers for higher
throughput.
...
**Benefits:** <50ms latency for users worldwide. Cache and read replicas in each region. Writes go to primary but reads
are local.
