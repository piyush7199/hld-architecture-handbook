...
Imagine a hospital with 100 doctors, each keeping their own patient notes. When a patient has a complex issue spanning multiple doctors, finding all relevant notes is nearly impossible.
...
**How It Works:**
```
Application → Logstash/Elasticsearch: Push logs directly
```
...
**How It Works:**
```
Filebeat → Log Files: Pulls logs from files
Filebeat → Logstash/Elasticsearch: Pushes to central system
```
...
**How It Works:**
```
Application → Kafka: Publishes logs
Logstash → Kafka: Consumes logs
...
→ High storage cost, slow queries
...
**Calculations:**
- **Daily Log Volume:** 100M × 500 bytes = 50 GB/day
- **90-Day Storage:** 50 GB × 90 = 4.5 TB (raw)
- **With Replication:** 4.5 TB × 2 (replicas) = 9 TB
...
**Architecture:**
```
Kubernetes Pods → Filebeat (sidecar) → Kafka → Logstash → Elasticsearch → Kibana
```
...
└─────────┼──────────────────┼──────────────────┼─────────┘
        ...
        └────────────────────┬────────────────────┘
                             │
        ┌────────────────────▼────────────────────┐
        │   Logstash Cluster (3 instances)        │
        │   ┌──────────────────────────────────┐ │
        │   │ - Kafka Input (20 consumers)     │ │
        │   │ - JSON Parsing                   │ │
        │   │ - Enrichment                     │ │
        │   │ - Elasticsearch Output           │ │
        │   └──────────────────────────────────┘ │
...
**Log Flow:**
```
1. Service → Log File: Writes JSON log
2. Filebeat → Log File: Tails file
3. Filebeat → Kafka: Publishes log to Kafka
4. Logstash → Kafka: Consumes log
5. Logstash: Parses, enriches log
6. Logstash → Elasticsearch: Indexes log
7. Elasticsearch: Stores in daily index
8. Kibana → Elasticsearch: Queries logs
9. User → Kibana: Views logs in dashboard
```
...
**Result:**
- ✅ Collects logs from 50 services (10K instances)
- ✅ Stores logs for 90 days (ILM policy)
- ✅ Fast search (full-text + structured queries)
- ✅ Handles 100M logs/day
- ✅ Real-time indexing (<30s)
- ✅ Trace correlation (trace_id search)
- ✅ Cost-optimized (open-source, ILM)
