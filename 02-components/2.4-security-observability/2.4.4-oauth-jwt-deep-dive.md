...
Imagine you want to use a photo printing service, but it needs access to your Google Photos. Instead of giving the
service your Google password (risky!), you use **OAuth 2.0** to grant limited access. Google shows you a consent
screen: "Photo Printing Service wants to view your photos. Allow?" You click "Allow," and Google gives the service a *
*token (JWT)** that proves you authorized it—without ever sharing your password.
...
**OAuth 2.0** is an authorization framework that allows applications to obtain limited access to user accounts on HTTP
services.
...
```
User → Client: "I want to access my photos"
Client → Authorization Server: "User wants access"
Authorization Server → User: "Do you allow this?"
User → Authorization Server: "Yes, allow"
Authorization Server → Client: "Here's a token"
Client → Resource Server: "Here's the token, give me photos"
Resource Server → Client: "Here are the photos"
```
...
```
1. User → Client: Clicks "Login with Google"
2. Client → Authorization Server: Redirects to authorization URL
   GET /authorize?client_id=xxx&redirect_uri=yyy&response_type=code&scope=photos
3. Authorization Server → User: Shows consent screen
4. User → Authorization Server: Clicks "Allow"
5. Authorization Server → Client: Redirects with authorization code
   GET /callback?code=abc123
6. Client → Authorization Server: Exchanges code for token
   POST /token
     ...
     client_secret=yyy
     redirect_uri=yyy
7. Authorization Server → Client: Returns access token + refresh token
   {
...
8. Client → Resource Server: Uses access token
   GET /api/photos
...
```
1. Client → Authorization Server: Request token
   POST /token
     grant_type=client_credentials
     client_id=xxx
     client_secret=yyy
     scope=api:read
2. Authorization Server → Client: Returns access token
   {
     "access_token": "eyJhbGc...",
     "expires_in": 3600,
     "token_type": "Bearer"
   }
3. Client → Resource Server: Uses access token
   GET /api/data
   Authorization: Bearer eyJhbGc...
```
...
```
1. Client → Authorization Server: Request token with username/password
   POST /token
     grant_type=password
...
**⚠️ Security Risk:** Client sees user password (only use for trusted apps)
...
```
1. Client → Authorization Server: Access token expired
2. Client → Authorization Server: Request new token
   POST /token
     grant_type=refresh_token
     refresh_token=def456
     client_id=xxx
     client_secret=yyy
3. Authorization Server → Client: Returns new access token
   {
     "access_token": "eyJhbGc...",
...
- Suspicious activity → Revoke refresh token
```
...
```
1. User → Client: Clicks "Login"
2. Client → Auth Server: Redirects to /authorize
   GET /authorize?
     client_id=app-123
...
3. Auth Server → User: Shows consent screen
4. User → Auth Server: Clicks "Allow"
5. Auth Server → Client: Redirects with code
   GET /callback?code=abc123&state=random-state
6. Client → Auth Server: Exchanges code for token
   POST /token
     grant_type=authorization_code
     code=abc123
     client_id=app-123
...
7. Auth Server → Client: Returns tokens
   {
     "access_token": "eyJhbGc...",
...
3. API Gateway → Resource Server: Forward request with user context
   X-User-ID: user123
...
Revocation Flow:
  1. User logs out → Add JWT jti to blacklist
  2. User changes password → Add all user tokens to blacklist
  3. Suspicious activity → Add token to blacklist
  4. Token validation → Check blacklist (Redis lookup)
```
...
```
1. Client → Auth Server: Access token expired
2. Client → Auth Server: Request new token
   POST /token
     grant_type=refresh_token
     refresh_token=refresh-abc-123
     client_id=app-123
...
4. Auth Server → Client: Returns new tokens
   {
     "access_token": "eyJhbGc...",
...
6. Client → Auth Server: Exchange code for JWT
7. Auth Server → Client: JWT access token + refresh token
8. Client → API Gateway: Request with JWT
9. API Gateway: Validates JWT (signature, claims, blacklist)
10. API Gateway → Resource Server: Request with user context
11. Resource Server → Client: Response
```
...
- ✅ Issues JWT tokens (stateless)
- ✅ Handles 10,000 logins/minute
- ✅ Supports refresh tokens (30 days, rotated)
- ✅ Token revocation (blacklist + database)
- ✅ Multi-tenant scopes
- ✅ High availability (99.99%)
