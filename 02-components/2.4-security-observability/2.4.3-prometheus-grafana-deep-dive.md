...
**Prometheus** is like having sensors on every machine that continuously measure and record data. **Grafana** is like a
control room dashboard that displays all these measurements in beautiful charts, so you can see trends, spot problems,
and get alerts.
...
Prometheus Server:
  ┌─────────────────┐
  │ Scrape Targets   │ (HTTP endpoints)
  │ Service Discovery│ (Kubernetes, Consul)
  │ Time-Series DB   │ (Local storage)
  │ PromQL Engine    │ (Query evaluation)
...
→ Use case: Total requests, errors, bytes transferred
```
...
→ Can go up or down
→ Use case: CPU usage, memory, queue size, active connections
```
...
→ Use case: Latency distribution, request sizes
```
...
→ Prometheus scrapes at configured interval (15s default)
```
...
```
Long-Term Storage:
  - Prometheus → Remote Write → InfluxDB, TimescaleDB, Thanos
...
Grafana:
  ...
  │ Dashboard Engine│ (Renders panels)
...
→ Use case: Request rate, latency trends
```
...
→ Current value, change from previous period
→ Use case: Total requests, error rate
```
...
→ Use case: Top endpoints by latency, service health
```
...
→ Shows value within range
→ Use case: CPU usage, memory usage
```
...
```
2D visualization
→ X-axis: time, Y-axis: buckets, Color: frequency
...
### ❌ Don't Use Prometheus & Grafana When:
...
→ Limited, meaningful labels
```

### ❌ **2. No Recording Rules**
...
→ Meaningful thresholds, different severities
```
...
- **Related Chapters:**
    ...
    - [2.1.17 Time Series Databases Deep Dive](../2.1-databases/2.1.17-time-series-databases-deep-dive.md) - Time-series
      storage
...
- **Storage:** 1M metrics/min × 90 days × 1.3 bytes = ~170 GB (compressed)
- **Scrape Interval:** 15 seconds (standard)

#### ✅ Step 1: Prometheus Architecture
...
50 services × 150 = 7,500 active time series
```
...
```
┌─────────────────────────────────────────────────────────┐
│          Kubernetes Cluster (50 Services)               │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
...
└─────────┼──────────────────┼──────────────────┼─────────┘
          ...
          └──────────────────┼──────────────────┘
                             │
        ┌────────────────────▼────────────────────┐
        │   Prometheus Cluster (3 instances)      │
        │  ┌──────────────────────────────────┐  │
        │  │ Prometheus 1 (17 services)        │  │
        │  │ Prometheus 2 (17 services)        │  │
        │  │ Prometheus 3 (16 services)        │  │
...
```
1. Grafana Dashboard → Query: rate(http_requests_total[5m])
2. Grafana → Prometheus (if <15 days) OR Thanos Query (if >15 days)
3. Prometheus/Thanos → Evaluates PromQL query
...
- ✅ Collects metrics from 50 services
- ✅ Stores metrics for 90 days
- ✅ Fast dashboard queries (<1s)
- ✅ Effective alerting (<1m latency)
- ✅ Automatic service discovery
- ✅ Handles 1M metrics/minute
