...
Redis (REmote DIctionary Server) is much more than a cache — it's an **in-memory data structure store** that can
function as a database, cache, message broker, and queue. Think of it as a super-fast, in-memory Swiss Army knife for
data that supports complex data structures (lists, sets, sorted sets, hashes) with atomic operations. Redis is
single-threaded but incredibly fast ($\text{O}(1)$ operations, < 1ms latency).
...
## ⚠️ Crucial Production Caveats & Operational Outage Risks

### 1. Single-Threaded Command Blocking (`KEYS *`, `FLUSHALL`, `HGETALL`)
- **Problem:** Because Redis processes commands sequentially on a single event loop, executing an $O(N)$ command like `KEYS *` or `HGETALL` on a collection with millions of items **blocks the entire Redis event loop**. All incoming requests freeze, driving application latency to infinity and tripping upstream circuit breakers!
- **Mitigation:** 
  1. Disable dangerous commands in production via `redis.conf`: `rename-command KEYS ""` and `rename-command FLUSHALL ""`.
  2. Use iterative cursor scanning (`SCAN`, `HSCAN`, `SSCAN`) instead of $O(N)$ fetch-all commands.

### 2. Linux Copy-On-Write (COW) Memory Spikes during RDB/AOF Forking
- **Problem:** When Redis creates a background RDB snapshot (`BGSAVE`) or rewrites the AOF file (`BGREWRITEAOF`), it calls `fork()` to create a child process. Linux relies on Copy-On-Write. If your application performs heavy writes during snapshotting, Linux duplicates physical memory pages.
- **Production Disaster:** If Redis memory usage is at $70\%$ RAM capacity and a write burst occurs during a `BGSAVE`, Copy-on-Write memory consumption can jump to $140\%$, triggering the **Linux OOM Killer** which violently slays the Redis process!
- **Mitigation:** Never allocate more than $50\%\text{--}60\%$ of machine RAM to Redis. Set `sysctl vm.overcommit_memory = 1`.

### 3. Cache Stampede (Thundering Herd Problem)
- **Problem:** When a high-traffic cache key expires (e.g., `homepage:trending_videos` hit by 50,000 QPS), thousands of application workers simultaneously experience a cache miss and hit the underlying RDBMS database concurrently, crashing the primary SQL cluster.
- **Mitigation:** Use **XFetch Algorithm (Probabilistic Early Expiration)** or Distributed Mutex Locks:
  $$\text{Re-compute if } -\beta \times \log(\text{rand}()) > \text{TTL} - \text{delta}$$
...
**Question:** How would you design this using Redis? What data structure would you use, and how would you handle the
massive write load (100K score updates/sec)?
