...
Imagine a bank with branches worldwide. Each branch needs to access the same account data, and all transactions must be
consistent—if you deposit money in New York, it should immediately appear in London. But the bank also needs to handle
...
**Key Insight:** Distributed SQL uses **consensus algorithms (Raft)** to ensure all nodes agree on data, enabling both
...
**Distributed SQL Databases** are databases that provide ACID transactions and SQL interface while scaling horizontally
across multiple nodes and regions.
...
Distributed SQL Database:
  ...
  │ Metadata Store  │ (Shard locations)
  │ Raft Groups     │ (Consensus per shard)
...
Write Flow:
  ...
  2. Leader → Followers: Replicate write
...
1. Prepare Phase:
   - Coordinator → All shards: "Prepare transaction"
   - Shards → Coordinator: "Prepared" or "Abort"

2. Commit Phase:
   - If all prepared → Coordinator: "Commit"
   - If any abort → Coordinator: "Abort"
...
- **Network:** Replication traffic (3× for 3 replicas)
...
### ❌ Don't Use Distributed SQL When:
...
### ❌ **1. Using Distributed SQL for Simple Use Case**
...
```
❌ Bad:
Single region, 100GB data → CockroachDB
→ Unnecessary complexity, higher cost

✅ Good:
Single region, 100GB data → PostgreSQL
→ Simple, cost-effective
```

### ❌ **2. Ignoring Write Amplification**

**Problem:** Not accounting for replication overhead

**Solution:** Plan for 3× network traffic

```
❌ Bad:
100K writes/sec → 100K network traffic
→ Actually needs 300K (3 replicas)

✅ Good:
100K writes/sec → 300K network traffic planned
→ Sufficient bandwidth provisioned
```
...
```
❌ Bad:
All writes to one shard → Bottleneck

✅ Good:
Monitor shard load → Auto-split hot shards → Even distribution
```
...
- **Related Chapters:**
    ...
    - [3.3.4 Distributed Database Challenge](../../../03-challenges/3.3.4-distributed-database/README.md) - Design
      challenge
...
- **Daily Data:** 1M × 1 KB = 1 GB/day
- **Annual Data:** ~365 GB/year
- **Replication:** 3× (3 regions) = ~1.1 TB total

#### ✅ Step 1: Database Choice
...
```
┌─────────────────────────────────────────────────────────┐
│          Applications (PostgreSQL Clients)                │
└──────────────────────┬──────────────────────────────────┘
                       │
        ┌──────────────▼──────────────┐
        │   CockroachDB Gateway        │
...
```
1. Client → Gateway: BEGIN TRANSACTION
2. Gateway: Routes to appropriate shards
3. Shard Leaders: Execute writes (Raft consensus)
4. Shard Leaders → Followers: Replicate writes
5. Followers → Leaders: Acknowledge
6. Leaders: Commit (majority = 2/3)
7. Leaders → Gateway: Success
8. Gateway → Client: COMMIT success
...
- ✅ High availability (99.99%)
- ✅ Low latency (<100ms reads, <200ms writes)
- ✅ SQL compatibility (PostgreSQL)
