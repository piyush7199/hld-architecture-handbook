...
Imagine you're searching for "a photo of a sunset over the ocean." Traditional databases would look for exact matches of
those words. But what if the photo is tagged "beach sunset" or "ocean horizon"? You'd miss it.

**Vector Databases** understand meaning, not just keywords. They convert text, images, or other data into **vectors** (
arrays of numbers) that capture semantic meaning. Similar meanings have similar vectors. So "sunset over ocean" and "
beach sunset" would have similar vectors and be found together.

**Key Insight:** Vector databases use **similarity search** (finding nearest neighbors) instead of exact matches. This
enables:
...
**Vector Embedding** is a numerical representation of data (text, image, audio) in a high-dimensional space where
...
→ These vectors are similar (close in vector space)
...
A **Vector Database** is a specialized database optimized for storing and querying high-dimensional vectors using
similarity search.
...
Traditional Database:
  ...
  → Misses "notebook computer" or "portable PC"

Vector Database:
  Query: Find products similar to "laptop" (vector search)
  → Returns: "laptop", "notebook computer", "portable PC"
...
- **Speed:** 100-1000× faster than brute force
...
→ Extract embedding (CNN features)
→ Search for similar embeddings
→ Return: Other sunset photos
```
...
→ Retrieve: "Return Policy: 30 days..."
...
### ❌ **1. Using Vector Database for Exact Matches**
...
→ Using vector search (overkill, slower)

✅ Good:
Query: "SELECT * FROM products WHERE name = 'laptop'"
→ Using PostgreSQL (exact match, fast)
```
...
→ Fast (searches only relevant subset)
```
...
→ Magnitude matters (wrong for text)

✅ Good:
Text embeddings with cosine similarity
→ Only direction matters (correct for text)
```
...
| **ANN Algorithms**  | 100-1000× faster search         | 5-10% accuracy loss                |
...
- Total storage: 10M × 3 KB = 30 GB (vectors only)
...
- **Embeddings:** 10M × 768 dimensions = 30 GB
...
- **Index Size:** HNSW index = 2-3× vector size = 60-90 GB

#### ✅ Step 1: Vector Database Choice
...
Collection: documents
  ...
  - Total: 10 shards × 2 replicas = 20 shard replicas
```
...
Query: Semantic search + filter by category
  ...
  - Result: 10× faster (searches 1M instead of 10M)
```
...
```
1. Client → Load Balancer: Semantic search query
2. Load Balancer → Query Node: Routes to available node
3. Query Node: Checks metadata filter (category = 'tech')
4. Query Node → Data Node: Vector search on filtered subset
5. Data Node: Searches HNSW index (<10ms)
6. Data Node → Query Node: Returns top-k results
7. Query Node → Client: Returns results + metadata
```
...
| **Metadata Filtering**  | 10× faster queries          | Requires metadata index               |
...
- **Throughput:** 1000 QPS (3 Query Nodes × 333 QPS each)
- **Accuracy:** 98% (vs brute force)
- **Update Latency:** <1 second (vector update), 5-10 min (index rebuild)

**Result:**

- ✅ Handles 10M document embeddings
- ✅ <100ms query latency (p95)
- ✅ Supports metadata filtering
- ✅ Real-time updates (<1s)
- ✅ Cost-optimized (tiered storage, open source)
