...
Imagine you want to call a friend, but you only know their name, not their phone number. You'd look them up in a phone book. **DNS (Domain Name System)** is the Internet's phone book—it translates human-readable domain names (like `google.com`) into IP addresses (like `142.250.191.14`) that computers use to connect.

**In distributed systems:**
- **DNS:** Hierarchical, distributed naming system
- **Goal:** Translate domain names to IP addresses
- **Benefit:** Humans use names, computers use IPs—DNS bridges the gap
...
**DNS (Domain Name System)** is a hierarchical, distributed naming system for computers, services, or resources connected to the Internet.
...
**How It Works:**
```
1. Client → Recursive Resolver: "What is www.example.com?"
2. Recursive Resolver → Root Server: "Where is .com?"
3. Root Server → Recursive Resolver: "Ask .com name servers"
4. Recursive Resolver → .com Name Server: "Where is example.com?"
5. .com Name Server → Recursive Resolver: "Ask example.com name servers"
6. Recursive Resolver → example.com Name Server: "What is www.example.com?"
7. example.com Name Server → Recursive Resolver: "192.0.2.1"
8. Recursive Resolver → Client: "192.0.2.1"
```
...
1. Client → Root Server: "Where is .com?"
2. Root Server → Client: "Ask .com name servers"
3. Client → .com Name Server: "Where is example.com?"
4. .com Name Server → Client: "Ask example.com name servers"
5. Client → example.com Name Server: "What is www.example.com?"
6. example.com Name Server → Client: "192.0.2.1"
```
...
```
Low TTL (60 seconds):
  ✅ Fast updates (failover, load balancing)
  ❌ More DNS queries (higher load, cost)

High TTL (86400 seconds = 24 hours):
  ✅ Fewer queries (lower load, cost)
  ❌ Slow updates (longer failover time)
```
...
US Client:
  www.example.com → 192.0.2.1 (US data center)

EU Client:
  www.example.com → 192.0.2.2 (EU data center)

APAC Client:
  www.example.com → 192.0.2.3 (APAC data center)
```
...
Health Check:
  ...
  - If unhealthy → Removes from DNS response
...
2. Chain of trust: Root → TLD → Domain
...
```
Traditional DNS:
  Client → DNS Server: Plain text query

DoH/DoT:
  Client → DNS Server: Encrypted query (HTTPS/TLS)
```
...
```
❌ Bad:
TTL: 1 second (updates instantly, but 3600× more queries)
...
US Region:
  www.example.com (US) → 192.0.2.1, 192.0.2.2, 192.0.2.3, 192.0.2.4, 192.0.2.5
  Weighted: 20% each (5 servers)

EU Region:
  www.example.com (EU) → 192.0.3.1, 192.0.3.2, 192.0.3.3, 192.0.3.4, 192.0.3.5
  Weighted: 20% each (5 servers)

APAC Region:
  www.example.com (APAC) → 192.0.4.1, 192.0.4.2, 192.0.4.3, 192.0.4.4, 192.0.4.5
  Weighted: 20% each (5 servers)

Default (Other locations):
  www.example.com (Default) → 192.0.2.1 (US primary)
```

**Routing Logic:**
```
User Location → Route 53 → Returns region-specific IPs
  - US user → US IPs
  - EU user → EU IPs
  - APAC user → APAC IPs
  - Unknown → Default (US)
...
Health Check Flow:
  ...
  2. If 3 failures → Mark server unhealthy
  3. Remove from DNS response
  4. If 1 success → Mark healthy, add back to DNS
```
...
#### ✅ Step 4: Load Balancing Within Region

**Weighted Routing:**
```
US Region (5 servers):
  www.example.com (US, weight: 20) → 192.0.2.1
  www.example.com (US, weight: 20) → 192.0.2.2
  www.example.com (US, weight: 20) → 192.0.2.3
  www.example.com (US, weight: 20) → 192.0.2.4
  www.example.com (US, weight: 20) → 192.0.2.5
...
Configuration:
  ...
  - Failover Time: <2 minutes (30s check × 3 failures + DNS propagation)
```
...
Example (US region fails):
  US user → EU servers (nearest backup)
  EU user → US servers (if EU fails, US backup)
```

#### ✅ Step 6: DNS Performance
...
Cache Invalidation:
  - Health check failure → Remove from DNS immediately
...
```
IPv4 Records (A):
  www.example.com (US) → 192.0.2.1, 192.0.2.2, ...

IPv6 Records (AAAA):
  www.example.com (US) → 2001:db8::1, 2001:db8::2, ...

Client Resolution:
  - IPv6-enabled client → Gets AAAA records
  - IPv4-only client → Gets A records
  - Dual-stack client → Prefers IPv6, falls back to IPv4
```
...
```
┌─────────────────────────────────────────────────────────┐
│          Global Users (1M queries/minute)                │
└──────────────────────┬──────────────────────────────────┘
                       │
        ┌──────────────▼──────────────┐
...
**Request Flow:**
```
1. User (US) → Route 53: DNS query for www.example.com
...
5. Route 53 → User: Returns 192.0.2.1, 192.0.2.2, 192.0.2.3, 192.0.2.4, 192.0.2.5
6. User → US Server: Connects to 192.0.2.1 (first in list)
```
...
3. Route 53 → User: Returns 192.0.2.2, 192.0.2.3, 192.0.2.4, 192.0.2.5
4. User → US Server: Connects to 192.0.2.2 (next healthy server)

If all US servers fail:
1. Route 53: All US servers unhealthy
2. Route 53: Failover to EU servers (secondary)
3. Route 53 → User: Returns EU servers
4. User → EU Server: Connects to EU (backup region)
```
...
- **Failover:** Primary region → Secondary region (automatic)
...
- **Failover Time:** <2 minutes (30s check × 3 + DNS propagation)
- **Health Detection:** <90 seconds (30s × 3 failures)

**Result:**
- ✅ Routes users to nearest data center (geolocation)
- ✅ Automatic failover (<2 minutes)
- ✅ Load balances within region (weighted routing)
- ✅ Health checks (removes unhealthy servers)
- ✅ Fast DNS resolution (<50ms)
- ✅ Handles 1M queries/minute
