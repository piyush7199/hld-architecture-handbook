...
Imagine a large office building with 50 different departments. Instead of visitors wandering around trying to find the right department, there's a **reception desk (API Gateway)** at the entrance. The receptionist:
...
- **Translates requests** ("I need help" → routes to Support Department)
...
An **API Gateway** is a reverse proxy that sits between clients and microservices, providing a unified interface for accessing multiple backend services.
...
```
X-Service-Version: v2 → Routes to v2 service
X-User-Type: premium → Routes to premium service
```

**Service Discovery Integration:**
```
API Gateway → Service Registry (Consul, etcd)
...
→ Protects each service independently
```
...
Client → API Gateway (REST/JSON)
      → Converts to gRPC/Protobuf
      → Backend Service (gRPC)
```

**HTTP to WebSocket:**
```
Client → API Gateway (HTTP upgrade request)
...
Client → API Gateway (GraphQL query)
      → Resolves to multiple REST calls
...
API Gateway:
  ...
  → Calls Order Service (GET /orders?user_id=123)
  → Aggregates responses
  → Returns combined profile
```
...
```
Mobile BFF:
  → Optimized for mobile (small payloads, offline support)
  → Returns minimal data

Web BFF:
  → Optimized for web (rich data, real-time updates)
  → Returns full data

Admin BFF:
  → Optimized for admin (bulk operations, analytics)
...
API Gateway:
  → Calls User Service (user info)
  → Calls Order Service (recent orders)
  → Calls Product Service (recommendations)
  → Combines responses
...
**Architecture:**
```
Client → Service-Specific Gateway → Service
```
...
**Architecture:**
```
Client → Central Gateway → Service Gateways → Services
```
...
→ Cannot access /api/user/* (unless also user)
```

**Attribute-Based Access Control (ABAC):**
```
User Attribute: department=engineering
→ Can access /api/engineering/*
→ Cannot access /api/sales/*
```

**Policy-Based:**
```
Policy: "Users can only access their own data"
→ API Gateway enforces: user_id from JWT must match resource user_id
```
...
### ❌ **1. API Gateway as Single Point of Failure**

**Problem:** Single API Gateway instance fails → entire system down

**Solution:** Multiple API Gateway instances with load balancer

```
❌ Bad:
Client → Single API Gateway → Services
(If gateway fails, all requests fail)

✅ Good:
Client → Load Balancer → Multiple API Gateways → Services
(If one gateway fails, others continue)
```

### ❌ **2. Bypassing API Gateway**
...
```
❌ Bad:
Client → Service (direct access)
Client → API Gateway → Service
(Inconsistent access, security issues)

✅ Good:
Client → API Gateway → Service (only path)
(All requests go through gateway)
```

### ❌ **3. Too Much Logic in API Gateway**
...
```
❌ Bad:
Client → API Gateway → User Service (every request)
(High latency, high backend load)

✅ Good:
Client → API Gateway → Cache (if available)
Client → API Gateway → User Service (if cache miss)
...
You are designing a microservices e-commerce platform with 20 services that must handle 10 million requests per day. The platform must:
...
- **Protocol:** REST (client) → gRPC (services)
- **Regions:** 3 regions

**Calculations:**
- **Peak QPS:** 1000 requests/second
- **API Gateway Instances:** 1000 QPS ÷ 500 QPS per instance = 2 instances minimum (use 5 for redundancy)
- **Rate Limiting Storage:** 1M users × 100 req/min = 100M rate limit counters (need Redis cluster)
...
**Architecture:**
```
Internet → Load Balancer → Multiple Kong Instances (5 per region)
                          ├─ Kong 1
                          ├─ Kong 2
                          ├─ Kong 3
                          ├─ Kong 4
...
```
Routes:
  /api/users/* → User Service (gRPC)
  /api/orders/* → Order Service (gRPC)
  /api/products/* → Product Service (gRPC)
  /api/payments/* → Payment Service (gRPC)
  /api/dashboard → Dashboard Aggregator (custom plugin)
```

**Service Discovery:**
```
Kong → Consul Service Registry
    → Discovers healthy service instances
    → Routes to available instances
...
**Flow:**
```
1. Client → Kong: Request with JWT token
...
5. Kong → Service: Forwards request with headers
6. Service: Trusts Kong (no re-validation needed)
```

**Token Caching:**
```
Kong → Redis: Cache validated tokens (TTL: 5 minutes)
→ Reduces validation overhead (signature verification)
→ 80% cache hit rate → 80% faster authentication
```
...
Kong gRPC Plugin:
  ...
  - Handles gRPC errors → HTTP status codes
```
...
Kong Transformation:
  → gRPC Call: GetUser(user_id: "123")
  → gRPC Service: user-service:50051
  → Protobuf Message: {user_id: "123"}

Service Response:
  → Protobuf: {id: "123", name: "John", email: "john@example.com"}
  → Kong: Converts to JSON
  → Client: Receives JSON response
```

#### ✅ Step 6: Request Aggregation (Dashboard)
...
Dashboard Response:
  ...
  - TTL: 60 seconds
  - Redis storage
  → Reduces service load (80% cache hit rate)
```

#### ✅ Step 7: Multi-Client Support (BFF Pattern)

**Backend for Frontend (BFF) Routes:**
```
Mobile BFF:
  /mobile/api/* → Mobile-optimized responses (minimal data)
  
Web BFF:
  /web/api/* → Web-optimized responses (rich data)
  
Admin BFF:
  /admin/api/* → Admin-optimized responses (bulk data, analytics)
```

**Response Transformation:**
```
Mobile Request: GET /mobile/api/users/123
  → Kong: Calls User Service
  → Kong: Filters response (only: id, name, avatar)
  → Client: Receives minimal payload (50% smaller)

Web Request: GET /web/api/users/123
  → Kong: Calls User Service
  → Kong: Returns full response (all fields)
...
Service Health:
  ...
  - Recovering: Gradually allow requests (1 → 10% → 50% → 100%)
```
...
**Request Flow:**
```
1. Client → Load Balancer: GET /api/users/123 (JWT token)
2. Load Balancer → Kong: Routes to available instance
...
6. Kong: Translates REST → gRPC
7. User Service → Kong: gRPC response
8. Kong: Translates gRPC → REST/JSON
9. Kong → Client: JSON response
```
...
- **Protocol Translation:** REST → gRPC (Kong gRPC plugin)
- **Aggregation:** Custom plugin for dashboard
- **Multi-Client:** BFF pattern (mobile, web, admin)

**Performance:**
- **Throughput:** 1000 requests/second (5 instances × 200 QPS each)
- **Latency:** <50ms added by gateway (JWT validation, routing)
- **Availability:** 99.99% (multi-region, circuit breakers, health checks)

**Result:**
- ✅ Routes requests correctly (path-based, service discovery)
- ✅ Handles authentication efficiently (JWT + caching)
- ✅ Implements rate limiting (per-user, per-service)
- ✅ Supports protocol translation (REST → gRPC)
- ✅ Aggregates data (dashboard plugin)
- ✅ Handles high traffic (1000 QPS, horizontal scaling)
