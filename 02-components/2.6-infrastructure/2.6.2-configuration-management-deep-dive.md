...
Imagine a large company with 1000 employees. Instead of each employee keeping their own phone directory, there's a **central directory (Configuration Management)** that everyone can access. When someone moves offices or changes their number, they update the directory once, and everyone automatically gets the new information.
...
**Configuration Management** is the process of storing, versioning, and distributing configuration data (settings, service locations, feature flags) across distributed systems.
...
GET /v3/kv/range
  Key: /services/user-service
  → Returns: {"host": "10.0.1.5", "port": 8080}
...
Consul Cluster:
  ...
  ┌──────▼──────┐
  │ Consul      │
  │ Server      │ (3-5 nodes, Raft consensus)
  │ - Stores    │
  │ - Coordinates│
  └─────────────┘
```
...
```
1. Service starts → Registers with Consul Agent
2. Consul Agent → Consul Server (registers service)
...
Vault Cluster:
  ┌─────────────┐
  │ Vault Server│ (3-5 nodes, active-standby)
  │ - Stores    │
  │ - Encrypts  │
  │ - Manages   │
  └──────┬──────┘
         │
  ┌──────▼──────┐
  │ Storage     │ (Consul, etcd, S3, etc.)
  │ Backend     │ (encrypted secrets)
  └─────────────┘
```
...
3. If successful → Service becomes leader
4. Leader renews TTL every 10 seconds (heartbeat)
5. If leader crashes → TTL expires → Other service becomes leader
```
...
5. If leader crashes → Session expires → Lock released
```
...
When key changes:
  etcd → Client: Notification (new value)
...
When service changes:
  Consul → Client: Notification (service added/removed)
...
```
1. Client → Load Balancer: Request
...
5. Load Balancer → Instance: Request
```
...
### ❌ Don't Use Configuration Management When:
...
### ❌ **1. Storing Secrets in Configuration Store**
...
→ Stored in plaintext, accessible to anyone with etcd access

✅ Good:
etcd: /app/database/password-path: "secret/database"
Vault: secret/database → password: "secret123" (encrypted)
→ Application reads path from etcd, retrieves secret from Vault
```
...
```
❌ Bad:
Service crashes → Still in service registry → Clients connect to dead service

✅ Good:
Service crashes → Health check fails → Removed from registry → Clients get healthy instances
```
...
→ Breaks when service moves or scales

✅ Good:
Code: user_service = consul.get_service("user-service")
→ Automatically gets current service location
```
...
**Calculations:**
- **Service Discovery Queries:** 10,000 instances × 10 queries/min = 100K queries/min
- **Configuration Reads:** 10,000 instances × 1 read/min = 10K reads/min
...
Client-Side Cache:
  ...
  → Reduces Consul load (90% cache hit rate)
```
...
Service Secret Cache:
  ...
  → Reduces Vault load (95% cache hit rate)
```
...
Leader Election Pattern:
  ...
  5. If leader crashes → Session expires → Lock released
  6. Another service acquires lock → Becomes new leader
```
...
Multiple Services:
  ...
  - If leader fails → Another service takes over (<30 seconds)
...
```
┌─────────────────────────────────────────────────────────┐
│          Kubernetes Cluster (10,000 Pods)                 │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │ Pod 1        │  │ Pod 2        │  │ Pod N        │  │
...
**Request Flow:**
```
1. Service starts → Consul Agent registers service
...
**Result:**
- ✅ Handles 10,000 service instances
