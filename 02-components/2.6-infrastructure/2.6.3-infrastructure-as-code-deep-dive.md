...
Imagine building a house. Instead of manually hiring contractors, buying materials, and coordinating everything
yourself, you write a **blueprint (Infrastructure as Code)** that describes exactly what you want. A system reads the
blueprint and automatically builds the house exactly as specified.
...
**Infrastructure as Code (IaC)** is the practice of managing and provisioning infrastructure through machine-readable
definition files, rather than through manual processes.
...
### ❌ Don't Use Infrastructure as Code When:
...
```
❌ Bad:
Developer → Direct terraform apply to production
→ No review
→ Risk of breaking production

✅ Good:
Developer → Git branch → Pull request → Review → Merge → Deploy
→ Code review
→ Automated testing
→ Safe deployment
```
...
- **Related Chapters:**
    ...
    - [2.6.2 Configuration Management Deep Dive](./2.6.2-configuration-management-deep-dive.md) - Configuration vs
      infrastructure
...
- **Total Resources:** 50 services × 3 envs × 3 regions = 450 service deployments
- **Infrastructure Components:** 450 × 3 (ECS, ALB, RDS) = 1,350 resources
- **State Management:** Need remote state with locking

#### ✅ Step 1: IaC Tool Choice
...
```
50 services × same module = Consistent infrastructure
→ Change module once → All services updated
...
#### ✅ Step 8: Preventing Manual Changes
...
│  │   └── modules/ (vpc, ecs-service, alb, rds)     │  │
│  └──────────────────────────────────────────────────┘  │
└──────────────────────┬──────────────────────────────────┘
                       │
        ┌──────────────▼──────────────┐
...
```
1. Developer → Git: Push infrastructure changes
2. GitHub Actions → Trigger: terraform validate, plan
3. Pull Request → Code review
4. Merge to main → Trigger: terraform apply (staging)
5. Manual approval → terraform apply (production)
6. Infrastructure → Updated in <30 minutes
```
...
- ✅ Supports 3 environments (dev, staging, prod)
- ✅ Handles 50 microservices (reusable modules)
- ✅ Multi-region deployment (3 regions)
- ✅ Automated provisioning (CI/CD pipeline)
- ✅ Version controlled (Git workflow)
- ✅ Prevents manual changes (IAM, AWS Config)
